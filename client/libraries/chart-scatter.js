...
(function (root, factory) {
	...
	var helpers = chartjs.helpers,
		hlp = {
			...
			ScatterPoint: chartjs.Point.extend({
				...
				// Custom inRange
				inRange: function (chartX, chartY, showCustomClickEvent) {
					var hitDetectionRange = this.hitDetectionRadius + this.radius * this.size;
					if (showCustomClickEvent) {
						return Math.abs(chartX - this.x) < Math.abs(hitDetectionRange);
					} else {
						return ((Math.pow(chartX - this.x, 2) + Math.pow(chartY - this.y, 2)) < Math.pow(hitDetectionRange, 2));
					}
				},
	...
	chartjs.ScatterDateScale = chartjs.ScatterNumberScale.extend({
		...
		_calculateDateScaleRange: function (valueMin, valueMax, drawingSize, fontSize) {
			...
			var maxSteps = drawingSize / (fontSize * 4.5);
	...
	chartjs.Type.extend({
		...
		initialize: function (datasets) {
			...
			//Set up custom showCustomClickEvent on the chart
			if (this.options.showCustomClickEvent) {

				helpers.bindEvents(this, this.options.tooltipEvents, function (evt) {
					if (evt.type == 'click') {
						var min = this.scale.xPadding + ((this.scale.xScaleRange.min - this.scale.xScaleRange.min) * (this.chart.width - this.scale.xPadding - this.scale.xPaddingRight) / (this.scale.xScaleRange.max - this.scale.xScaleRange.min));
						var max = this.scale.xPadding + ((this.scale.xScaleRange.max - this.scale.xScaleRange.min) * (this.chart.width - this.scale.xPadding - this.scale.xPaddingRight) / (this.scale.xScaleRange.max - this.scale.xScaleRange.min));

						this.options.showCustomClickEventCallback(this.scale.dataRange.xmin + ((evt.offsetX - min) / (max - min)) * (this.scale.dataRange.xmax - this.scale.dataRange.xmin));
					}


					// console.log(this.chart.canvas.clientWidth);
					// // this.scale.calculateY(this.scale.yScaleRange.min)
					// console.log(evt.clientX, evt.clientY);

					// var activePoints = (evt.type == 'click') ? this.getPointsAtEvent(evt) : [];
					// if (activePoints.length > 0) {
					// 	this.options.showCustomClickEventCallback(activePoints[0].arg);
					// }
			...
			if (this.options.showCustomScrollEvent) {
				helpers.bindEvents(this, this.options.tooltipEvents, function (evt) {
					if (evt.type == 'mousewheel') {
						this.options.showCustomScrollEventCallback(evt.deltaY);
						evt.preventDefault();
					}
					// console.log(evt.deltaY);


					// var activePoints = (evt.type == 'mousewheel') ? this.getPointsAtEvent(evt) : [];
					// if (activePoints.length > 0) {
					// 	this.options.showCustomScrollEventCallback(activePoints[0].arg);
					// }
		...
		getPointsAtEvent: function (e) {
			...
			helpers.each(this.datasets, function (dataset) {
				helpers.each(dataset.points, function (point) {
					if (point.inRange(eventPosition.x, eventPosition.y, that.options.showCustomClickEvent)) pointsArray.push(point);
				});
			}, this);
			return pointsArray;
		},
		...
		draw: function (ease) {
			...
			if (this._hasData()) {
				...
				if (this.options.timecurrent) {
					var xpos = this.scale.xPadding + ((this.options.timecurrent - this.scale.xScaleRange.min) * (this.chart.width - this.scale.xPadding - this.scale.xPaddingRight) / (this.scale.xScaleRange.max - this.scale.xScaleRange.min));
					var ctx = this.chart.ctx;
					ctx.beginPath();
					ctx.moveTo(xpos, 0);
					ctx.lineTo(xpos, this.scale.calculateY(this.scale.yScaleRange.min) + 60);
