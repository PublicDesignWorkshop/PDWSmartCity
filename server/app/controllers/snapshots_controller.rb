class SnapshotsController < ApplicationController
  ...
  before_action :search_snapshots, only: [:search]
  ...
  private
    ...
    def snapshot_params
      params.require(:snapshot).permit(:location_id, :timestamp, :timeinterval, :sensors, :comments, :questions, :answers)
    end

    def search_snapshots
      @snapshots = Snapshot.order(timestamp: :asc).where(location_id: params[:location_id])
    end
end
